...
<task type="auto">
  ...
  <action>
Step 0 - Create the verify script at tests/_verify_07_03.py with the following content (the file
MUST exist before the verify command runs). Use the Write tool or write it via Python open():
...
IMPORTANT: The locked signature from CONTEXT.md is:
    add_annotation(fig, x, y=None, text: str = '', col: str = None)
Note that y comes BEFORE text, and text has a default of ''.
  ...
  <done>
    - tests/_verify_07_03.py exists and runs without assertion errors
    - _apply_plotly_annotations helper exists in plots.py
    - add_annotation() function exists with signature (fig, x, y=None, text='', col=None)
      and can be imported from pxts.plots
...
<verification>
The verify script tests/_verify_07_03.py is created in Task 1 Step 0 before it is invoked. Run:

    cd /c/Users/adria/Desktop/pxts
    python -c "exec(open('tests/_verify_07_03.py').read())"
...
It asserts:
  ...
  - add_annotation(fig, "2024-01-03", text="Event") adds annotation to existing figure
...
- add_annotation() function available as pxts.add_annotation with signature (fig, x, y=None, text='', col=None)
